...
public:
    ...
    memory_arena_new(const memory_arena_new&) = delete;
    memory_arena_new& operator=(const memory_arena_new&) = delete;
    memory_arena_new(memory_arena_new&&) = delete;
    memory_arena_new& operator=(memory_arena_new&&) = delete;
...
private:
    ...
    static void destroy_node(N* node) noexcept {
        delete node;
    }
    ...
    void push_free_slot(u32 idx) noexcept {
        u64 head =
            freelist_head_.load(std::memory_order_relaxed);
